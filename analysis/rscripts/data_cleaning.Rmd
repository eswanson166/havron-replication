...
We also add the AOIs for this experiment. Note that this is based on the size of the specific videos we used (400 pixels wide and 226 pixels high). Since the Webgazer measures are messy, we add 150 pixels of padding around each edge of the video.
...
et_data$vid1_left_x <- margin_left - 150
et_data$vid1_right_x <- margin_left + 400 + 150
et_data$top_y <- margin_top - 150
et_data$bottom_y <- margin_top + 226 + 150
```
...
```{r}
et_data$vid2_left_x <- margin_left + 880 - 150
et_data$vid2_right_x <- margin_left + 1280 + 150
```
